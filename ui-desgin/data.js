// Shared data for Ledarise prototype
// Product imagery uses solid-tone backgrounds with SKU codes — placeholder until real photography lands.

window.LEDARISE_DATA = (function () {
  ...
  // Hand-curated featured products with editorial copy
  const products = [
    {
  ...
  // Customers — sampled from the 9,027 imported orders
  const customers = [
  ...
  // Orders — pulled from the imported set; enough for a realistic table
  const STATUSES = ['complete', 'processing', 'pending', 'cancelled'];
  const orderSeeds = [
    { no: 'PO000418277', cust: 0, items: [[0,1]], sub: 289, ship: 25, status: 'complete', date: '2026-04-22' },
    { no: 'PO000418421', cust: 2, items: [[2,1],[5,1]], sub: 938, ship: 45, status: 'complete', date: '2026-04-21' },
    { no: 'PO000418502', cust: 4, items: [[8,2]], sub: 378, ship: 25, status: 'processing', date: '2026-04-25' },
    { no: 'PO000418517', cust: 11, items: [[11,1]], sub: 599, ship: 45, status: 'complete', date: '2026-04-26' },
    { no: 'PO000418601', cust: 3, items: [[3,1]], sub: 269, ship: 25, status: 'complete', date: '2026-04-15' },
    { no: 'PO000418645', cust: 8, items: [[6,1],[1,1]], sub: 658, ship: 25, status: 'complete', date: '2026-04-23' },
    { no: 'PO000418709', cust: 5, items: [[4,1]], sub: 219, ship: 25, status: 'pending', date: '2026-04-27' },
    { no: 'PO000418815', cust: 1, items: [[7,1]], sub: 459, ship: 45, status: 'processing', date: '2026-04-26' },
    { no: 'PO000418892', cust: 6, items: [[3,1]], sub: 269, ship: 25, status: 'complete', date: '2026-04-12' },
    { no: 'PO000418951', cust: 9, items: [[0,1],[8,1]], sub: 478, ship: 25, status: 'complete', date: '2026-04-17' },
    { no: 'PO000419023', cust: 7, items: [[5,1]], sub: 389, ship: 45, status: 'complete', date: '2026-04-20' },
    { no: 'PO000419088', cust: 10, items: [[9,1]], sub: 309, ship: 25, status: 'cancelled', date: '2026-04-19' },
    { no: 'PO000419145', cust: 11, items: [[2,1],[11,1]], sub: 1148, ship: 45, status: 'processing', date: '2026-04-27' },
    { no: 'PO000419201', cust: 0, items: [[10,1]], sub: 369, ship: 25, status: 'complete', date: '2026-04-25' },
    { no: 'PO000419244', cust: 8, items: [[1,1]], sub: 339, ship: 25, status: 'pending', date: '2026-04-28' },
  ];

  const orders = orderSeeds.map((s, i) => {
    const customer = customers[s.cust];
    const items = s.items.map(([pi, q]) => ({
      product: products[pi], qty: q, subtotal: products[pi].price * q
    }));
    return {
      id: i + 1,
      orderNo: s.no,
      ...
      date: s.date,
      shipAddress: `${Math.floor(Math.random()*999)+100} ${['Country Club','Park Ave','Main St','Oak Lane','Westminster Rd'][i%5]}, ${customer.city}`,
    };
  });

  // 30 day revenue trend (US 76.8%, UK 17.3%, DE 5.9%)
  const revenueTrend = [];
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    const noise = 0.85 + Math.random() * 0.4;
  ...
  return {
    ...
    revenueTrend, countryDist, topSkus,
    COLORS, LENGTHS, MATERIALS, CATEGORIES, STATUSES,
    stats: {
      totalOrders: 9027,
